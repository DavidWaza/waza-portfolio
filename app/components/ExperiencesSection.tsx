...
const ExperienceSection = ({ data }: { data: WorkExperienceProps[] }) => {
  console.log("dattttss", data);

  // Helper function to parse job_responsibility
  const parseResponsibilities = (responsibility: string | string[]): string[] => {
    if (Array.isArray(responsibility)) {
      return responsibility;
    }
    // If it's a JSON string, parse it
    try {
      const parsed = JSON.parse(responsibility);
      return Array.isArray(parsed) ? parsed : [responsibility];
    } catch {
  ...
  return (
    <section className="bg-[#f7f8f3] px-4">
      <div className="max-w-5xl mx-auto space-y-10 bg-white p-6 sm:p-8 lg:p-10 rounded-2xl shadow-sm">
        {data?.map((exp, index) => {
          const responsibilities = parseResponsibilities(exp.job_responsibility);
                ...
                <div className="lg:flex items-center gap-4">
                  ...
                  <div className="py-3 lg:py-0">
                    ...
                    <p className="text-[#21493e] text-sm sm:text-base">
                      {exp.company} · {exp.date_started} - {exp.date_ended}
                    </p>
                  </div>
                </div>
                {/* Job Type */}
